...
func (p *Systray) AppendMenu(label string, onclick func()) {
    p.Menu = append(p.Menu, &MenuItemSystray{Label: label, OnClick: onclick})
}
 
// AppendSeparator to the menu.
func (p *Systray) AppendSeparator() {
    p.Menu = append(p.Menu, &MenuItemSystray{Separator: true})
}
...
func (p *Systray) SetBalloonClick(fn func()) {
    p.balloonClick = fn
}
...
func (p *Systray) displayMenu(menuItems []*MenuItemSystray) error {
